...
Activity event publishing is **merged but not deployed**. The CloudEvents publisher reached `main` on 2026-09-12 (`fbba207`), and the running image predates it: the deployment runs `agent-builder:model-fallback-20260427211530`, an April build. Verified against the binary rather than the merge date — `strings` on `/root/agent-builder` in the running pod returns **zero** matches for `CloudEvents publisher enabled`, `tas.activity.agents`, `KAFKA_BROKERS` and `aether-shared/go-events`, and neither pod logs the enabled *or* the disabled startup line because neither exists in that build. `KAFKA_BROKERS=kafka-shared-0.kafka-shared.tas-shared.svc.cluster.local:9092` **is** correctly set via `ConfigMap/agent-builder-config`, so the only thing missing is a rebuild and redeploy. The general lesson still holds, and this is an example of it: what runs is not pinned to a commit, so read the deployed binary rather than the merge date when you need to know what is live.
