...
> [!WARNING] The pin is what `apply -k` deploys, not merely a default. It was corrected in PR #16 to track the deployed image, and the running image was restored at 21:16 UTC — both pods log `CloudEvents publisher enabled` again. Keep the pin equal to what is actually deployed, or `apply -k` is a downgrade. The `model-migration` CronJob still runs the April build and will move to the same image on the next apply.
