...
Activity event publishing is **deployed and running**. The CloudEvents publisher reached `main` on 2026-09-12 (`fbba207`) and the running pods log `CloudEvents publisher enabled — emitting tas.activity.agents to [kafka-shared-0.kafka-shared.tas-shared.svc.cluster.local:9092]` at start, against `KAFKA_BROKERS` from `ConfigMap/agent-builder-config`.

> [!WARNING] `k8s/kustomization.yaml` pins an image, and that pin is what `apply -k` deploys. On 2026-09-21 it still read `model-fallback-20260427211530` — an April build predating the publisher — while production ran `tas-agent-builder:latest`, which contains it. A routine `apply -k` for an unrelated replica-spread change rolled production back five months at 19:07 UTC and **stopped activity event publishing for about two hours** until it was noticed and reverted at 21:16. The pin has been corrected. Keep it equal to what is actually deployed, or `apply -k` is a downgrade. An earlier version of this file recorded the post-rollback state as "merged but not deployed" — that was measured twelve minutes after the accidental rollback and described the damage as the status quo.
...
- **Resolved 2026-09-21 (PR #13).** `k8s/secret.yaml` had held literal credential values in Git, and the `JWT_SECRET` was not merely "real-looking" — it was the live one (SEC-22). The file is deleted, replaced by `k8s/secret.example.yaml` carrying placeholders and a generation recipe, and removed from `kustomization.yaml` so `apply -k` can no longer overwrite the live Secret. The secret has been rotated; the old committed value no longer authenticates.
...
Secrets by location. `Secret/agent-builder-secret` in namespace `tas-agent-builder` holds exactly five keys: `DB_USER`, `DB_PASSWORD`, `JWT_SECRET`, `ROUTER_API_KEY`, and `DEEPLAKE_API_KEY`. `ROUTER_API_KEY` is optional — `validateConfig` deliberately does not require it, and the client omits the `Authorization` header when it is empty (`services/impl/router_service_impl.go:103`). No provider credential belongs in this service; OpenAI and Anthropic keys live with the LLM Router. Read values with `kubectl get secret agent-builder-secret -n tas-agent-builder`. The checked-in `k8s/secret.yaml` no longer exists — it was deleted in PR #13 and replaced by `k8s/secret.example.yaml`, which holds placeholders only and is excluded from the kustomization.
